...
def pred_frames(data, model, metric_X, metric_Y, frames, prev_frames, tile_manhattan_error, act_tiles, pred_tiles, count):
	...
	for k in range(len(frames)):
		...
		end_pred = time.time()
		with open(topic + '_pred.txt','a') as f:
			f.write(str(end_pred - start_pred)+'\n')
...
def build_model(data, frame_nos, max_frame, tot_objects):
	...
	while True:
		...
		end_fit = time.time()
		with open(topic + '_fit.txt','a') as f:
			f.write(str(end_fit - start_fit)+'\n')
...
def alloc_bitrate(pred_tiles, frame_nos, chunk_frames, pref_quality):
	...
	end_alloc = time.time()
	with open(topic + '_pred.txt','a') as f:
		f.write(str(end_alloc - start_alloc)+'\n')
	return vid_bitrate
...
def main():
	...
	print(manhattan_error,x_mae,y_mae)
	i = 0
	while True:
		curr_frame=frame_nos[i]
		if curr_frame<5*fps:
